...
module.exports = function (jsonapi, data, opts) {
  ...
  function findIncluded(relationshipData, relationshipName, from, fromPath) {
    return new Promise(function (resolve) {
      ...
      var currentPath = [
        ...
        "id_" + relationshipData.id,
      ];

      var path = (fromPath || []).concat(currentPath);
      ...
      if (included) {
        return Promise
          .all([extractAttributes(included), extractRelationships(included, currentPath)])
  ...
  function extractRelationships(from, fromPath) {
        ...
        if (relationship.data === null) {
          ...
          return Promise
            .all(relationship.data.map(function (relationshipData) {
              return extractIncludes(relationshipData, key, from, fromPath);
            }))
            .then(function (includes) {
              if (includes) { dest[keyForAttribute(key)] = includes; }
            });
        } else {
          return extractIncludes(relationship.data, key, from, fromPath)
  ...
  function extractIncludes(relationshipData, relationshipName, from, fromPath) {
    return findIncluded(relationshipData, relationshipName, from, fromPath)
