...
module.exports = function (jsonapi, data, opts) {
  ...
  function findIncluded(relationshipData, includes) {
    return new Promise(function (resolve) {
      if (!includes || !relationshipData) { resolve(null); }

      var included = _find(includes, {
        id: relationshipData.id,
        type: relationshipData.type
      });

      if (included) {
        // To prevent circular references, remove include from the collection before passing it forward
        var remainingIncludes = [...includes];
        var includeIndex = remainingIncludes.indexOf(included);

        if (includeIndex > -1) {
          remainingIncludes.splice(includeIndex, 1);
        }

        return Promise
          .all([extractAttributes(included), extractRelationships(included, remainingIncludes)])
  ...
  function extractRelationships(from, includes) {
        ...
        if (relationship.data === null) {
          ...
          return Promise
            .all(relationship.data.map(function (relationshipData) {
              return extractIncludes(relationshipData, includes);
            }))
            .then(function (include) {
              if (include) { dest[keyForAttribute(key)] = include; }
            });
        } else {
          return extractIncludes(relationship.data, includes)
            .then(function (include) {
              if (include) { dest[keyForAttribute(key)] = include; }
            });
        }
  ...
  function extractIncludes(relationshipData, includes) {
    return findIncluded(relationshipData, includes)
      .then(function (include) {
  ...
  this.perform = function () {
    return Promise
      .all([extractAttributes(data), extractRelationships(data, jsonapi.included)])
      .then(function (results) {
        var attributes = results[0];
        var relationships = results[1];
